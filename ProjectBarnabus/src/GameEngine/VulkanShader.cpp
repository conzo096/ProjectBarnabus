...
bool VulkanShader::Link()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout[i]) != VK_SUCCESS)
		{
		...
		if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline[i]) != VK_SUCCESS)
		{
...
VkRenderPass VulkanShader::GetRenderPass()
{
	return renderPass;
}
...
void VulkanShader::CreateDepthResources()
{
	...
	CreateImage(renderer->GetSwapChainExtent().width, renderer->GetSwapChainExtent().height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory);
	depthImageView = CreateImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
}

// Use Vulkan Texture class.
void VulkanShader::CreateTextureImage()
{
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load("res\\Textures\\test.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void VulkanShader::CreateTextureImageView()
{
